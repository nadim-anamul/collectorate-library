<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">{{ __('ui.add_category') }}</h2>
            ...
            <form action="{{ route('admin.categories.store') }}" method="POST" class="space-y-8">
                ...
                <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('ui.slug') }} *</label>
                                <input id="slug" name="slug" value="{{ old('slug') }}" required
                                    class="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 dark:bg-gray-700 dark:text-white transition-all duration-200"
                                    placeholder="{{ __('ui.slug_placeholder') }}">
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('ui.slug_description') }}</p>
                            </div>
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('ui.description') }}</label>
