<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">{{ __('ui.tags') }}</h2>
                        ...
                        <table class="min-w-full text-sm">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr class="text-left text-gray-600 dark:text-gray-300">
                                    ...
                                    <th class="px-6 py-3 font-medium">{{ __('ui.slug') }}</th>
                                    ...
                                    <td class="px-6 py-3">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <form action="{{ route('admin.tags.destroy',$tag) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
                                                <button class="inline-flex items-center px-3 py-1.5 text-sm text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition duration-150" onclick="return confirm('{{ __('ui.delete_tag_confirm') }}')">
                                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
